...
from . import config
from .data_provider import get_ohlcv_deep
...
def _manipulation_stop(
    ...
    la entrada, para no depender de "pips" fijos que no aplican igual a forex/cripto/
    indices/materias primas en un mismo dashboard). `at_idx` permite fijar el ATR en
    un punto historico especifico (backtest); por defecto usa la ultima vela (uso en
    vivo)."""
...
def detect_dxy_divergence(df_pair: pd.DataFrame, df_dxy: pd.DataFrame, lookback: int = 5) -> dict | None:
    """Compara barridas de liquidez recientes entre el DXY y un par (pensado para
    EUR/USD, inversamente correlacionados). Si el DXY barre un maximo (rechazo
    bajista) sin que el par barra el minimo correspondiente en una ventana de tiempo
    cercana (o viceversa), hay divergencia: el par muestra fortaleza/debilidad
    relativa que confirma el sesgo esperado por la correlacion inversa."""
    dxy_sweeps = detect_liquidity_sweeps(df_dxy, lookback=lookback)
    pair_sweeps = detect_liquidity_sweeps(df_pair, lookback=lookback)
    if not dxy_sweeps:
        return None

    last_dxy_sweep = dxy_sweeps[-1]
    dxy_time = df_dxy.index[last_dxy_sweep["idx"]]
    if len(df_pair) > 1:
        bar_span = df_pair.index[-1] - df_pair.index[-2]
    else:
        bar_span = pd.Timedelta(minutes=15)
    window = bar_span * 10

    expected_pair_kind = "bullish" if last_dxy_sweep["kind"] == "bearish" else "bearish"
    matching = None
    for s in pair_sweeps:
        if s["kind"] != expected_pair_kind:
            continue
        pair_time = df_pair.index[s["idx"]]
        if abs((pair_time - dxy_time).total_seconds()) <= window.total_seconds():
            matching = s
            break

    divergence = matching is None
    confirmed_bias = ("alcista" if last_dxy_sweep["kind"] == "bearish" else "bajista") if divergence else None

    return {
        "dxy_sweep": last_dxy_sweep,
        "dxy_sweep_time": dxy_time,
        "pair_matching_sweep": matching,
        "divergence": divergence,
        "confirmed_bias": confirmed_bias,
    }


def in_high_liquidity_session(timestamp) -> bool:
    """True si el timestamp cae en la sesion de Londres o Nueva York (mayor liquidez)."""
...
def to_new_york_time(timestamp) -> datetime:
    ...
    hora de Nueva York usada en todo el dashboard (UTC-4 fijo, misma convencion que
    las ventanas de sesion — no ajusta por horario de invierno EST/UTC-5). Se usa
    para mostrar "a que hora estan actualizados los datos", no la hora del reloj
    local en que corrio el script."""
    ts = pd.Timestamp(timestamp)
...
def zone_probability(
    ...
    analogos historicos (mismo tipo, altura de zona similar en proporcion al ATR —
    sustituye el "+/-5 pips" fijo del indicador de referencia, que no aplica igual a
    forex/cripto/indices/materias primas) y con el desempeño reciente de los ultimos
    `recent_n` quiebres de estructura del mismo tipo. No replica un algoritmo
    propietario: es una aproximacion estadistica fiel al metodo descrito."""
    if atr_value <= 0:
        return None
...
def build_zone_signal(mtf_prob: dict) -> dict | None:
    ...
    (`zone["tp"]`, el mismo dato que usa el motor SMC institucional) fraccionado en
    `config.PROBABILITY_SIGNAL_TP_FRACTIONS` niveles de toma de parcial — TP5
    coincide exactamente con ese objetivo. El SL sale del borde opuesto de la zona
    mas un colchon de ATR (`config.ICT_MANIPULATION_SL_ATR_MULT`, la misma
    convencion que usa el stop de manipulacion en el resto del sistema).
...
def cached_multi_timeframe_probability(source: str, symbol: str, exchange: str | None, lookback: int = 5) -> dict:
    """Version cacheada (120s) de multi_timeframe_probability para yfinance/ccxt —
    recorrer ~5000 velas en 3 temporalidades es pesado para repetir en cada tick
    de un fragment."""
    df_1h = get_ohlcv_deep(source, symbol, "1h", exchange)
    df_15m = get_ohlcv_deep(source, symbol, "15m", exchange)
    df_5m = get_ohlcv_deep(source, symbol, "5m", exchange)
