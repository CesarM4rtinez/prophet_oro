...
wrapper propio sobre `requests`, verificado contra la documentacion publica de la API v1.
...
class CapitalComClient:
    ...
    def browse_markets(
        ...
        Si `search_term` viene vacio, Capital.com no ofrece un endpoint de "listar todos
        los instrumentos" sin termino de busqueda, asi que se combinan varias busquedas
        "semilla" representativas de la clase (ver config.CAPITAL_PRESET_SEEDS) para
        aproximar un listado amplio, deduplicado por epic.
        """
        if search_term:
    ...
    def get_candles(self, epic: str, resolution: str = config.DEFAULT_CAPITAL_RESOLUTION, max_points: int = 100) -> pd.DataFrame:
        ...
        # horaria (ej. "2026-08-05T23:45:00") — utc=True los marca como tz-aware en vez
        # de naive, igual que ya hace `_parse_created_date` para las posiciones. Sin
        # esto, cruzar estas velas con otra fuente tz-aware (ej. el DXY via yfinance en
        # `detect_dxy_divergence`) revienta con "Cannot subtract tz-naive and tz-aware".
    ...
    def update_position(self, deal_id: str, stop_level: float | None = None, profit_level: float | None = None) -> str:
        """Modifica el stop/take-profit de una posicion ya abierta (ej. mover a break-even)."""
        payload = {}
        if stop_level is not None:
            payload["stopLevel"] = stop_level
        if profit_level is not None:
